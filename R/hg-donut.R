...
hg_donut <- function(data,
  ...
  color_by <- var_cat[1]
  data_viz <- colors_data(data_viz, var_cat = var_cat, var_num = var_num, viz = "donut", color_by = color_by, ...)

  data_viz <- hg_list(data_viz, hdtype, "donut")
...
#' @export
hg_donut_Cat <- function(data, dic = NULL, ...) {
  vars <- data_vars(data)
  hg_donut(data, dic, var_cat = vars[1], ...)
}
